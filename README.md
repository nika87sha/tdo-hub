...
│   ├── auto-triage.sh      # Clasificador de inbox
...
- **B**: rofi de una línea → se añade `- HH:MM idea` al `brain_dump/dump_HOY.md`.
  `daily-routine` las cuenta como "ideas sin triar" y `auto-triage` las clasifica.
- **T**: `auto-triage.sh --brain` — clasifica solo el brain_dump.
